...
export function createMcpServer(opts: McpServerOptions = {}) {
  const server = new Server(
    { name: 'paperid-mcp', version: '0.1.0' },
    { capabilities: { tools: {}, resources: {} } }
  );

  const client = new PaperIdClient({
    phone:     opts.phone     ?? process.env.PAPERID_PHONE,
    password:  opts.password  ?? process.env.PAPERID_PASSWORD,
    token:     opts.token     ?? process.env.PAPERID_TOKEN,
    companyId: opts.companyId ?? process.env.PAPERID_COMPANY_ID,
    userId:    opts.userId    ?? process.env.PAPERID_USER_ID,
  });
